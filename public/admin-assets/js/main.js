...
$(document).ready(function() {
    function updateBulkUI() {
        ...
        if (checkedCount > 0) {
            ...
            $('#bulkActionContainer').css('display', 'flex'); // Hiện cụm action
        } else {
            $('#bulkActionContainer').hide();
        }
    }

    // Sự kiện check/uncheck như cũ
    $('#checkAll, .check-item').on('change', function() {
    ...
    $('#applyBulkAction').on('click', function() {
        ...
        var selectedIds = $('.check-item:checked').map(function() { return $(this).val(); }).get();
        ...
        var confirmMsg = action === 'delete' ? 'Bạn có chắc muốn xóa?' : 'Bạn có muốn khôi phục?';
        
        if (confirm(confirmMsg + ' (' + selectedIds.length + ' mục)')) {
            $.ajax({
                url: url,
                type: 'POST',
                data: {
                    action: action, // Gửi hành động: delete hoặc restore
                    ids: selectedIds,
